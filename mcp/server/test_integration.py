...
import mcp_pb2
import mcp_pb2_grpc
from server import Config, MCPServicer, AuthInterceptor
...
def _start_server(cfg_path: str):
    ...
    stub = mcp_pb2_grpc.MachineControlProxyStub(channel)
...
class TestAuth:
    def test_missing_token_rejected(self, server_env):
        ...
        with pytest.raises(grpc.RpcError) as exc_info:
            stub.RunCommand(
                mcp_pb2.RunCommandRequest(
                    target_host="loopback", command="echo", args=["hi"]
                )
            )
...
# ---------------------------------------------------------------------------
# RunCommand — core acceptance criterion
# ---------------------------------------------------------------------------

class TestRunCommand:
    def test_run_command_echo_loopback(self, server_env):
        """Core acceptance criterion: agent calls RunCommand on loopback, gets output."""
        stub, _ = server_env
        resp = stub.RunCommand(
            mcp_pb2.RunCommandRequest(
        ...
        assert "hello mcp" in resp.stdout
        assert resp.proxy_error == ""

    def test_run_command_alias_resolves(self, server_env):
        """Host alias 'local' should resolve to the same loopback host."""
        stub, _ = server_env
        resp = stub.RunCommand(
            mcp_pb2.RunCommandRequest(
                target_host="local",
        ...
        assert resp.exit_code == 0
        assert "alias-ok" in resp.stdout

    def test_run_command_allowlist_blocks(self, server_env):
    ...
    def test_run_command_unknown_host(self, server_env):
        stub, _ = server_env
        with pytest.raises(grpc.RpcError) as exc_info:
            stub.RunCommand(
                mcp_pb2.RunCommandRequest(
...
class TestAsyncJobs:
    ...
    def test_run_build_returns_job_handle(self, server_env):
        stub, _ = server_env
        handle = stub.RunBuild(
            mcp_pb2.RunBuildRequest(
                target_host="loopback",
                build_command="echo",
                build_args=["build-ok"],
                timeout_seconds=10,
    ...
    def test_run_build_job_succeeds(self, server_env):
        stub, _ = server_env
        handle = stub.RunBuild(
            mcp_pb2.RunBuildRequest(
                target_host="loopback",
                build_command="echo",
                build_args=["build-success"],
                timeout_seconds=10,
            ),
            metadata=_auth_meta(),
        )
        job = self._wait_for_job(stub, handle.job_id)
        assert job.status == mcp_pb2.JOB_SUCCEEDED
