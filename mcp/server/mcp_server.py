...
class MCPServicer(mcp_pb2_grpc.TestflingerControlServicer):
    ...
    def BuildDebianPackage(self, request: mcp_pb2.BuildDebianPackageRequest, context):
        host = self._resolve_host(request.target_host, context)
        if host is None:
            return mcp_pb2.JobHandle()

        job = new_job(1, request.target_host)  # kind=1: BUILD
        ...
        def run():
            ...
            # Build script: clone repo, checkout ref, run dpkg-buildpackage
            repo_url = request.repo_url
            git_ref = request.git_ref or "HEAD"
            build_script = (
                f"set -e; "
                f"tmpdir=$(mktemp -d); "
                f"git clone --depth 1 {repo_url!r} \"$tmpdir\"; "
                f"cd \"$tmpdir\"; "
                f"git fetch origin {git_ref!r} && git checkout FETCH_HEAD; "
                + (f"cd {workdir!r}; " if workdir else "")
                + "dpkg-buildpackage -us -uc -b"
            )

            if host.get("use_ssh", False):
                _, _, rc, proxy_err = execute_ssh(
                    host, "bash", ["-c", build_script],
                    None, env, timeout, job,
                )
            else:
                _, _, rc, proxy_err = execute_local(
                    "bash", ["-c", build_script],
                    None, env, timeout, job,
                )
